...
def convert_torch2transformers(torch_path, transformers_path):
    ...
    model_params = sum(
        p.numel() for p in lm_model.parameters() if p.requires_grad
    )
...
def push_to_hf(export_model_path):
    def init_model():
        ...
        model = AutoModelForCausalLM.from_pretrained(
            export_model_path, trust_remote_code=True
        )
...
if __name__ == '__main__':
    ...
    torch_path = (
        f"../out/rlhf_{lm_config.dim}{'_moe' if lm_config.use_moe else ''}.pth"
    )
