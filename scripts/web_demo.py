...
selected_model = st.sidebar.selectbox(
    'Models', list(MODEL_PATHS.keys()), index=0
)  # 默认选择 MiniMind2
...
def main():
        ...
        if message['role'] == 'assistant':
            with st.chat_message('assistant', avatar=image_url):
                ...
                if st.button('×', key=f'delete_{i}'):
                    ...
                    st.session_state.chat_messages = (
                        st.session_state.chat_messages[: i - 1]
                    )
                    st.rerun()
        else:
            st.markdown(
    ...
    if prompt:
        ...
        with st.chat_message('assistant', avatar=image_url):
            ...
            x = torch.tensor(
                tokenizer(new_prompt)['input_ids'], device=device
            ).unsqueeze(0)
            with torch.no_grad():
                ...
                try:
                    for y in res_y:
                        ...
                        placeholder.markdown(
                            process_assistant_content(answer),
                            unsafe_allow_html=True,
                        )
                ...
                with st.empty():
                    if st.button('×', key=f'delete_{len(messages) - 1}'):
                        ...
                        st.session_state.chat_messages = (
                            st.session_state.chat_messages[:-2]
                        )
                        st.rerun()
