...
def init_model(args):
    ...
    if args.load == 0:
        ...
        if args.lora_name != 'None':
            apply_lora(model)
            load_lora(
                model, f'../{args.out_dir}/{args.lora_name}_{args.dim}.pth'
            )
    else:
    ...
    print(
        f'MiniMind模型参数量: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1e6:.2f}M(illion)'
    )
        ...
        with torch.no_grad():
            ...
            for y in res_y:
                answer = tokenizer.decode(
                    y[0].tolist(), skip_special_tokens=True
                )
    ...
    try:
        if request.stream:
            ...
            with torch.no_grad():
                ...
                answer = tokenizer.decode(
                    res_y.squeeze()[x.shape[1] :].tolist(),
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--load', default=0, type=int, help='0: 从原生torch权重，1: 利用transformers加载'
    )
    parser.add_argument(
        '--model_mode',
        default=1,
